...
def produce_data_to_kafka(producer, topic, data):
    """Produce data to Kafka with error handling."""
    try:
        producer.produce(
            topic,
            key=str(data['id']),
        ...
        print(f"Buffer full for topic {topic}. Retrying...")
        time.sleep(1)
        produce_data_to_kafka(producer, topic, data)  # Retry logic
    except Exception as e:
        print(f"Failed to produce record to {topic}: {e}")
